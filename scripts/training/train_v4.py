...
LR_BACKBONE     = 5e-5
WEIGHT_DECAY    = 1e-3
EPOCHS          = 6           # 8x more data per epoch → fewer epochs needed
WARMUP_EPOCHS   = 1
BATCH_SIZE      = 24
NUM_WORKERS     = 2
UNFREEZE_BLOCKS = 4
VAL_FRAC        = 0.20
CHIPS_TRAIN     = 6           # 6 chips × 3 checkpoints = 18 per county-year per epoch
CHIPS_VAL       = 12
USE_BF16        = True
HUBER_DELTA     = 10.0
ZERO_THRESH     = 0.50
SEED            = 42
OUT_DIR         = ROOT / "models" / "checkpoints" / "prithvi_yield_v4"
LOG_PATH        = ROOT / "reports" / "training_logs" / "train_v4_log.csv"
